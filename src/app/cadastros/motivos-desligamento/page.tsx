...
export default function MotivosDesligamentoPage() {
  const {
    ...
    handleSalvar,
    resetFormAndLoad,
  } = useCrud<MotivoDesligamento>({ endpoint: "/motivo-desligamento" });
  const {
    register,
    handleSubmit,
    reset,
  ...
  const handleEdit = (item: MotivoDesligamento) => {
    setValue("MotDescricao", item.MotDescricao);
  ...
  const [pendingData, setPendingData] = useState<MotivoFormData | null>(null);
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  className={`p-2 w-full rounded border ${
                    errors.MotDescricao ? "border-red-500" : "border-gray-300"
                  }`}
                />
                {errors.MotDescricao && (
                  <span className="text-red-500 text-xs">
                    Descrição do motivo é obrigatória!
                  </span>
                )}
              </div>
            </div>
